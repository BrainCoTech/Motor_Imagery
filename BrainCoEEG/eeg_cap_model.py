...
async def get_addr_port():
    ...
    (addr, port) = ("192.168.3.3", 53129)  # junhan
...
def set_env_noise_cfg(type, fs: float = 250):
    return bc_device_sdk.set_env_noise_cfg(type, fs)


def remove_env_noise(data: list, channel: int):
    return bc_device_sdk.remove_env_noise_notch(data, channel)
    # return bc_device_sdk.remove_env_noise_sosfiltfilt(data, channel)
...
def perfrom_impendance_filter(data: list, channel: int):
    ...
    # return eeg_cap.apply_impendance_sosfiltfilt(data, channel)
